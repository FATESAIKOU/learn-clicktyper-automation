...
def process_file(
    ...
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="輸出檔案路徑"),
...
def calc(
    ...
    if operation not in operations:
        typer.echo(f"錯誤: 不支援的操作 '{operation}'。支援的操作: {', '.join(operations.keys())}", err=True)
        raise typer.Exit(1)
...
def search_replace(
    ...
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="輸出檔案路徑"),
    case_sensitive: bool = typer.Option(True, "--case-sensitive/--ignore-case", help="區分大小寫")
):
    """
    在檔案中搜尋並替換文字
    """
    try:
        ...
        count = content.count(search_term) if case_sensitive else len(re.findall(re.escape(search_term), content, re.IGNORECASE))
...
def generate_report(
    ...
    file_pattern: str = typer.Option("*.py", "--pattern", "-p", help="檔案模式 (例如: *.py, *.txt)"),
    output_format: str = typer.Option("text", "--format", "-f", help="輸出格式 (text/json)")
):
    """
    生成目錄中檔案的統計報告
    """
    ...
    try:
        ...
        if output_format == "json":
            ...
            for file_info in report_data["檔案詳情"]:
                typer.echo(f"  • {file_info['檔案名']} ({file_info['行數']} 行, {file_info['大小(bytes)']} bytes)")
...
def interactive_demo():
    ...
    if typer.confirm("要設置一個示範密碼嗎？", default=False):
        password = typer.prompt("請輸入密碼", hide_input=True, confirmation_prompt=True)
        typer.echo("✅ 密碼設置成功！")
    ...
    # 使用 typer.style 添加顏色
    success_msg = typer.style("✨ 示範完成！感謝您的參與！", fg=typer.colors.GREEN, bold=True)
